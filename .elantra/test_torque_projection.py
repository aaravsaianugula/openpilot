...
class TestBands(unittest.TestCase):
    ...
    def test_the_low_speed_boundaries_are_band_boundaries(self):
        # "under 20 mph" has to be readable straight off the table. 8.94 m/s IS 20 mph, so if
        # it falls inside a band every number for the low-speed regime is contaminated by
        # frames from above it. Holding these splits still also keeps scans comparable.
