...
def _chunk_url(base_url: str, chunk_file_name: str, index: int, count: int) -> str:
  if chunk_file_name:
    return urljoin(base_url, chunk_file_name)

  from openpilot.common.file_chunker import get_chunk_name
  return get_chunk_name(base_url, index, count)
    ...
    try:
      ...
      if not expected_hash and len(artifact.chunks) > 0:
        ...
        for i, chunk in enumerate(artifact.chunks):
          ...
          if not chunk.sha256 or not await verify_file(chunk_path, chunk.sha256):
            chunks_valid = False
            break
      ...
      if len(artifact.chunks) > 0:
        ...
        for i, chunk in enumerate(artifact.chunks):
          chunk_path = get_chunk_name(full_path, i, len(artifact.chunks))
          if chunk.sha256 and not await verify_file(chunk_path, chunk.sha256):
            raise ValueError(f"Hash validation failed for chunk {i+1} of {filename}")
        if expected_hash and not await verify_file(full_path, expected_hash):
