...
class ModelState(ModelStateBase):
  ...
  def run(self, bufs: dict[str, VisionBuf], transforms: dict[str, np.ndarray],
    ...
    if self._combined_model_type == 'supercombo':
      ...
      for i, policy_slices in enumerate(self._policy_slices_list):
        ...
        if ('off' in self._policy_keys[i]
          ...
          and any('plan' in self._policy_slices_list[j] for j, key in enumerate(self._policy_keys) if 'on' in key.lower())):
    ...
    # Older compiled models keep recurrence in feat_q. A CPU round trip avoids QCOM queue corruption.
